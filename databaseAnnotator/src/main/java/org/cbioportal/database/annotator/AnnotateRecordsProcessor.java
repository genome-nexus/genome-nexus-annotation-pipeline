...
public class AnnotateRecordsProcessor implements ItemProcessor<MutationEvent, MutationEvent>{
    ...
    @Value("#{jobParameters[ignoreOriginalGenomicLocation]}")
    ...
    @Value("#{jobParameters[noteColumn]}")
    ...
    public MutationEvent process(MutationEvent i) throws Exception {
        ...
        try {
            annotatedRecord = annotator.annotateRecord(record, true, isoform, true, stripMatchingBases, ignoreOriginalGenomicLocation, addOriginalGenomicLocation, noteColumn);
        }
