...
public class BatchConfiguration {
    ...
    @Value("${chunk:10000000}")
    private String chunk;

    @Bean
    public Job annotationJob() {
        return jobBuilderFactory.get(ANNOTATION_JOB).start(step()).build();
    }

    @Bean
    public Step step() {
        return stepBuilderFactory.get("step").<AnnotatedRecord, String>chunk(Integer.parseInt(chunk)).reader(reader()).processor(processor()).writer(writer()).build();
    }
